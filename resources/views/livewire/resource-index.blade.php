<ul
    ...
    @foreach ($indexRows ?? $this->indexRows() as $row)
        @if ($this->showIndexGroups && !$this->treeview() && $this->orderBy && $this->getAttribute($this->orderBy)->type != 'number' && strlen($char = ucfirst(mb_substr($this->hasTranslation($attribute) ? $row[$this->orderBy][app()->getLocale()] ?? reset($row[$this->orderBy]) : $row[$this->orderBy] ?? '', 0, 1))) && (empty($last) || $last !== $char))
