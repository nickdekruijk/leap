...
- **The code editor no longer escapes the slashes in what it stores.** A field in JSON
  mode is pretty-printed on save, and without `JSON_UNESCAPED_SLASHES` every address in
  it came back as `https:\/\/example.com`: valid JSON, and not what the person who typed
  the address wrote. Unicode was already left alone for the same reason. The stored value
  parses to exactly what it did before, so nothing that reads it changes; a record has to
  be saved once for its own copy to be rewritten.

## [1.17.2] — 2026-09-09

### Fixed

- **The slug input actually shapes what is typed into it.** The handler called its own
  helper as `this.clean()`, and in an Alpine `x-on` expression `this` is not the
  component: the expression is evaluated against the data stack, so the call threw
  "this.clean is not a function" on every keystroke and the field accepted a space like
  any other. It worked in `x-data`, where `this` does bind, which is why it read as
  correct. A test now walks every Alpine expression in the views and refuses `this.`
  inside one.
